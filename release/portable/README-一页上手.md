...
`komorebi` 可以把桌面上的窗口按固定方式自动排布，并把这些窗口组织进不同的工作空间里，方便你用快捷键快速切换。

这个 portable 版本默认已经开启 `whitelist_mode` 和 `force_manage`，思路不是“自动接管所有窗口”，而是**由你主动挑选想用的窗口，把它们组织进自己的工作空间**。
